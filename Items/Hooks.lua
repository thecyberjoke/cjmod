...
<<<<<<< HEAD
    ...
    if G.trinkets and G.trinkets.cards then
        for n, x in pairs(G.trinkets.cards) do
            if x.calculate then
                x.calculate(x, context)
            end
        end
    end
    ]]
...
function G.UIDEF.card_focus_ui(card)
    print("!")
    local res = uidef(card)

    if (card.area == G.trinkets and G.trinkets) and G.STATE ~= G.STATES.TUTORIAL then
        local attach = res:get_UIE_by_ID('ATTACH_TO_ME')
        ...
        if attach then
            attach.children.sell = G.UIDEF.card_focus_button{
                card = card, parent = attach, type = 'sell',
